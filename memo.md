...
- ネットワークをより深くすると...
    - MNISTで精度が上がった
    - 他にも、アンサンブル学習、学習係数の減衰、データ拡張などの手法を使うことで、認識精度の向上が望める。
    - Data Augmentation
        - データを増やすために、データを変形させること
        - 画像の回転、平行移動、拡大縮小など
    - 層を深くすると、より少ないパラメータで高い表現力を得られる。
        - 受容野 (ニューロンに変化を生じさせる局所的な空間領域) を広くカバーできること
- ディープラーニングの小歴史
    - ILSVRC (ImageNet Large Scale Visual Recognition Challenge)
        - 大規模画像認識のコンペをきっかけに、ディープラーニングが注目されるようになった
    - ImageNet
        - 1000クラスの1400万枚をこえるデータセット
        - Deep Learning を使うことで精度が大幅に向上した
    - VGG
        - 重みのある層 (畳み込み層、全結合層) を16層に重ねた
        - 3x3のフィルタを使っている
    - GoogLeNet
        - インセプション構造: サイズの異なるフィルタを複数適用してその結果を重ねること
    - ResNet
        - スキップ構造: 残差学習 = 入力をそのまま出力に足すことで、勾配消失を防ぐ
    - 転移学習: 事前学習済みのモデルの重みを初期値として再学習すること
- 高速化
    - 畳み込み演算がボトルネックになる
    - CPUではなくて、GPUを使うことで高速化できる
    - 分散学習: 複数のGPUやマシンを使って学習すること。Google の TensorFlow や Microsoft の CNTK などがある。
    - 演算精度のビット削減
        - 32bit 浮動小数点数を 16bit 浮動小数点数にすることで、メモリの使用量を減らすことができる
        - ニューラルネットワークはロバスト性があるため、問題ない
- 実用例
    - 物体検出
        - 画像の中で物体のバウンディングボックスを検出して、クラス分類をする
    - セグメンテーション
        - 画像の中で物体の領域をピクセル単位で、クラス分類する
    - 画像キャプション生成
        - 画像を見て、その内容を自然言語で説明する
        - CNN と RNN を組み合わせて実現する
-　他の技術
    - 画像スタイルの変換
    - 画像生成
        - DCGAN (Deep Convolutional Generative Adversarial Network)
            - Generator と Discriminator を使って、画像を生成する
    - 自動運転
    - Deep Q-Network
        - 報酬を使った強化学習  